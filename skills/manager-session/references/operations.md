...
Worker prepares from the trusted state, then verifies identity, current submission and exact Manager target before one authorized native `send_message_to_thread` call using the non-fixture `hostRequest`. Keep actual tool evidence; the plan itself is always not-sent. An uncertain send is not permission to blindly resend or start a timer.
