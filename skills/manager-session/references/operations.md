...
Set the intended title in the authorized creation request (`create_thread.title` in this host). Treat that result as provisional: this host can display an optimistic title even when the creation-time save fails. A matching sidebar title or read result alone is not persistence evidence.

After each new task resolves to a verified exact hostId/threadId, explicitly call the native rename operation (`set_thread_title`) with the intended title, even if it already looks correct. This host's explicit rename requires persistence acknowledgement. Naming is complete only after that call succeeds and a subsequent native `read_thread` returns the intended title for the same formal task. Retain the acknowledgement with the original creation/activation evidence; do not add fields to runtime state. Pending client IDs must first resolve. Finalizing the requested name is part of authorized creation, not permission to rename unrelated tasks.

If renaming is unavailable, denied or uncertain, keep the original task and report naming as unconfirmed. For an explicit metadata-not-ready failure such as `rollout ... is empty`, retry the same rename at most once after setup is verified ready, or reconcile it on the next foreground continuation. Do not retry a denial or overwrite a later user-selected title. Never recreate a task, send a prompt to force metadata creation, edit app storage, or start a polling timer to repair naming. Pending naming does not change verified identity or block otherwise authorized work; do not claim the name was saved until the acknowledgement and read-back are confirmed.
