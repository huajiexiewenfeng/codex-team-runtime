...
export function render(v,{roundPages=[],codexLinks=false,live=false,embedded=false}={}) {
 if(typeof codexLinks!=='boolean')throw new Error('Invalid codexLinks option');
 if(typeof live!=='boolean')throw new Error('Invalid live option');
 if(typeof embedded!=='boolean'||(embedded&&!live))throw new Error('Invalid embedded option');
 const liveKey=value=>live?` data-live-key="${esc(value)}"`:'';
 const taskId=(t,index)=>live?`task-${Buffer.from(JSON.stringify([t.roundId,t.id])).toString('base64url')}`:`task-${index}`;
 ...
 const sharedConversationHelp=entries.some(e=>e.href)?`<details class="conversation-help"${liveKey('conversation-help')}><summary>兼容入口 · 不保证锁定主机</summary><p id="conversation-help" class="source">${conversationHelp}</p></details>`:'';
 ...
 const member=(m,index)=>{
  ...
  return `<article class="member"${liveKey(`member-${m.id}`)}><div class="member-top"><span class="avatar ${m.role==='Manager'?'manager':''}" aria-hidden="true">${esc(m.role==='Manager'?'M':m.role==='Liaison'?'L':'W')}</span><div><h3>${esc(m.name)}</h3><p class="source">${esc(m.role)} · ${esc(m.lifecycle)}</p></div></div><p>${duty}</p>
   ${owned.length?`<ul class="assignments">${owned.map(({t,i})=>`<li><a href="#${taskId(t,i)}">${esc(t.id)} · ${esc(labels[t.status])}</a><span class="source">${elapsed(t.elapsedMs)}</span></li>`).join('')}</ul>`:''}
   ...
   <details${liveKey(`member-details-${m.id}`)}><summary>查看绑定身份</summary><p class="mono">memberId: ${esc(m.id)}<br>hostId: ${esc(m.binding.hostId)}<br>threadId: ${esc(m.binding.threadId)}${m.binding.pendingId?`<br>pendingId: ${esc(m.binding.pendingId)}`:''}</p></details>
  </article>`;
 };
 ...
 <a href="#main" class="skip">跳到工作区</a>${embedded?'':`<header class="top"><div class="brand"><span class="brand-mark" aria-hidden="true">tr</span>codex team runtime</div><div class="top-meta"><span class="mono">${esc(v.team.id)}</span><span class="pill neutral">${live?'最新工作台 · 只读':'只读快照'}</span></div></header>`}
 ...
 <main id="main"><section id="overview"><div class="source-banner">${v.sourceKinds.includes('fixture')?'模拟来源 / FIXTURE · 包含离线示例，不代表真实团队成功。':live?'自动同步团队记录 · 未连接 Agent 实时遥测。':'来源记录快照 · 未连接实时宿主。'}<span>页面不会发送消息、创建任务或改变业务状态。</span></div>
 ...
 <section class="panel"><h2>成本与模型</h2><p>${embedded?'Token 用量与 MCP 调用请查看顶部「指标统计」页签。':'模型与 Token 用量：未接入'}</p><p class="source">任务历时不是模型计算时间。未采集的数据不按 0 计算，也不推算节省比例。</p></section></aside></div>
 <footer id="snapshot"><strong>${live?'最新工作台 · 按页面请求同步记录':'固定快照 · 不会自行刷新'}</strong><p>源版本 ${v.sourceVersion} · 状态更新时间 ${time(v.sourceUpdatedAt)}<br>计算时间 ${time(v.asOf)} · 全部时间以 UTC 标示${live?`<br>Registry 版本 ${esc(v.registry?.teamRevision??'未关联 Registry')}`:''}</p><p class="mono">快照 ID ${esc(v.snapshotId)}</p><p>任务历时包含等待，不代表实际模型计算耗时。${live?'同步成功不代表成员刚刚汇报。历史轮次显示该轮次当前记录；需要不可变历史证据时，另行导出离线快照。':'查看新进展需重新导出；HTML 与对应 JSON 快照来自同一份数据。'}</p></footer>
 </main></div></body></html>`;
}
